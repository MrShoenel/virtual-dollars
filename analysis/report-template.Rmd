...
Also, let's factor in the amount of missed meetings.
...
scoreSMissed <- ecdf(allScores$S_missed)
scoreOMissed <- ecdf(allScores$O_missed)

# Here, a higher value is worse, so we need to calculate 1-ecdf:
allScores$S_missedScore <- sapply(allScores$S_missed, function(x) {
  if (x == 0) return(1)
  return(1 - scoreSMissed(x))
})
allScores$O_missedScore <- sapply(allScores$O_missed, function(x) {
  if (x == 0) return(1)
  return(1 - scoreOMissed(x))
})
```

```{r}
weightMissedMeetings <- 0.5
```

We compute the total score by factoring in the amount of missed meetings with a weight, because these are absences with given notice. That weight is `r round(weightMissedMeetings, 3)`.

```{r}
allScores$scoreMiss <- (allScores$S_missedScore + allScores$O_missedScore) / 2

allScores$total <- (allScores$score + weightMissedMeetings * allScores$scoreMiss) / (1 + weightMissedMeetings)
```
