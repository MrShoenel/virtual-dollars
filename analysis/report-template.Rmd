...
Also, let's factor in the amount of missed meetings. However, these we'll compute differently. With every additionally missed meeting, the score should decrease exponentially. We calculate the score therefore as `1.1**(-n*1.4**n)`, where `n` is the number of missed meetings.
...
allScores$S_missedScore <- sapply(allScores$S_missed, function(x) 1.1**(-x*1.4**x))
allScores$O_missedScore <- sapply(allScores$O_missed, function(x) 1.1**(-x*1.4**x))
